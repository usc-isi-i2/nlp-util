...
public final class Scoreds {
  ...
  private static class MapEntryToIntegerToScored<T>
      implements Function<Map.Entry<T, Integer>, Scored<T>> {
  ...
  private static class MapEntryToDoubleToScored<T>
      implements Function<Map.Entry<T, Double>, Scored<T>> {
  ...
  /**
   * Comparator which compares Scoreds first by score, then by item, where the item ordering to use
  ...
  /**
   * Comparator which compares by scores only. Beware: using this comparator in things which depend
   * on the comparator for equality judgements (e.g. SortedSet) may result in things with the same
   * score being considered equal.
   */
