...
/**
 * Creates a Guice module from a parameter. The parameter is expected to contain the class name of
 * the module or modules to create. You may specify a default class or classes to be used if the
 * parameter is absent. The modules must be constructable either by a zero-argument constructor, one
 * which takes a single {@link Parameters} object, one which takes a single annotation object, or
 * one which takes both a parameters and an annotation.
 *
 * <p>If created with {@link #forParameter(String)}, a single class name will be expected as the
 * parameter value and a single module will be created.
 *
 * <p>If created with {@link #forMultiParameter(String)}, multiple class names will be allowed and
 * the returned module will be an anonymous module which simply installs all the created modules.
 * This is useful in conjunction with Guice Multibindings.
 *
 * <p>Being able to pass an annotation in to the constructor can be useful when you may want to use
 * the same module to bind multiple sites (e.g. binding multiple string normalizers by using the
 * same modules with different annotations).
 *
 * @author Ryan Gabbard
 */
public final class ModuleFromParameter {
  ...
  private ModuleFromParameter(
    ...
    checkArgument(
        defaultModule == null || this.defaultClasses.isEmpty(),
        "May not specify both default module classes and a default module instance");
    this.allowMultiple = allowMultiple;
    checkArgument(
        allowMultiple || this.defaultClasses.size() < 2,
        "Cannot have multiple defaults " + "if multiple bindings are not allowed");
  }
  ...
  public ModuleFromParameter withDefault(Class<?> defaultClass) {
    return new ModuleFromParameter(
        parameter,
        ImmutableList.<Class<?>>of(defaultClass),
        defaultModule,
        annotationClass,
        allowMultiple);
  }
  ...
  public ModuleFromParameter withDefault(Module defaultModule) {
    return new ModuleFromParameter(
        parameter, defaultClasses, defaultModule, annotationClass, allowMultiple);
  }

  public ModuleFromParameter withDefaults(Iterable<? extends Class<?>> defaultClasses) {
    checkState(
        allowMultiple, "Cannot have multiple defaults if multiple bindings " + "are not permitted");
    return new ModuleFromParameter(
        parameter, defaultClasses, defaultModule, annotationClass, allowMultiple);
  }
  ...
  public ModuleFromParameter withAnnotation(Class<? extends Annotation> annotation) {
    return new ModuleFromParameter(
        parameter, defaultClasses, defaultModule, annotation, allowMultiple);
  }
      ...
      try {
        modulesB.add(
            ModuleUtils.classNameToModule(
                parameters, clazz, Optional.fromNullable(annotationClass)));
