 ...
 *
 * <p>Equality and hashcode for {@link Multitable}s is delegated to their {@link #cellSet()}s.
 *
 * <p>Adapted from {@link Table}.
 *
...
public interface Multitable<R, C, V> {
  ...
  /**
   * Returns {@code true} if the table contains a mapping with the specified value. May run in O(n)
   * or O(1) time depending on the implementation.
   *
  ...
  /**
   * Returns the values corresponding to the given row and column keys, or an empty collection if no
   * such mapping exists.
   *
   * @param rowKey key of row to search for
   * @param columnKey key of column to search for
   ...
   * cell views, as returned by {@link #cellSet}, are equal. In general, two Multitables with
   * identical key-key-value mappings may or may not be equal, depending on the implementation. For
   * example, two SetMultitable instances with the same key-value mappings are equal, but equality
   * of two ListMultitable instances depends on the ordering of the values for each key.
   *
   * <p>A non-empty SetMultitable cannot be equal to a non-empty ListMultitable, since their asMap()
   * views contain unequal collections as values. However, any two empty multitables are equal,
   * because they both have empty asMap() views.
   */
  @Override
   ...
   * value mapping in the table with that row key, the returned multimap associates the column key
   * with the values. If no mappings in the table have the provided row key, an empty multimap is
   * returned.
   *
   * <p>Changes to the returned multimap will update the underlying table, and vice versa.
   *
   ...
   * value mapping in the table with that column key, the returned multimap associates the row key
   * with the values. If no mappings in the table have the provided column key, an empty multimap is
   * returned.
   *
   * <p>Changes to the returned multimap will update the underlying table, and vice versa.
   *
  ...
  /**
   * Returns a view that associates each row key with the corresponding multimap from column keys to
   ...
   *
   * <p>In contrast, the multimaps returned by {@code rowMap().get()} have the same behavior as
   * those returned by {@link #row}. Those multimaps may support {@code setValue()}, {@code put()},
   * and {@code putAll()}.
   *
  ...
  /**
   * Returns a view that associates each column key with the corresponding multimap from row keys to
   * values. Changes to the returned map will update this table. The returned map does not support
   * {@code put()} or {@code putAll()}, or {@code setValue()} on its entries.
   *
   * <p>In contrast, the multimaps returned by {@code columnMap().get()} have the same behavior as
   * those returned by {@link #column}. Those multimaps may support {@code setValue()}, {@code
   * put()}, and {@code putAll()}.
   *
