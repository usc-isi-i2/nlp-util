 ...
 *
 * <p>You might think that this is unnecessary because you can simply build your map using e.g. a
 * {@link java.util.HashMap} and then call {@link ImmutableMap#copyOf(Iterable)}, but that loses
 * {@link ImmutableMap.Builder}'s very nice determinism guarantees.
 *
 * <p>These builders are generally less efficient than {@link ImmutableMap.Builder}, so be cautious
 * using them in performance-sensitive code.
 */
public interface LaxImmutableMapBuilder<K, V> {
  ...
  LaxImmutableMapBuilder<K, V> putAll(
...
final class MonotonicLaxImmutableMapBuilder<K, V> implements LaxImmutableMapBuilder<K, V> {
  ...
  public LaxImmutableMapBuilder<K, V> put(K key, V value) {
    ...
    if (null == existingMapping) {
      ...
      throw new IllegalArgumentException(
          "Refusing to add two mappings for the key "
              + key
              + ": "
              + existingMapping
              + " and "
  ...
  @Override
  public LaxImmutableMapBuilder<K, V> putAll(
      Iterable<? extends Map.Entry<? extends K, ? extends V>> entries) {
