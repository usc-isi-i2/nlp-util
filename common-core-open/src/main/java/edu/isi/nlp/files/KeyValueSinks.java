...
public final class KeyValueSinks {
   ...
   *
   * <p>Compression should generally be used unless the values are very small or already compressed.
   *
   ...
   */
  @Nonnull
  public static KeyValueSink<Symbol, byte[]> forPalDB(
      final File dbFile, final boolean compressValues) throws IOException {
    return PalDBKeyValueSink.forFile(dbFile, compressValues);
  }

  /**
   * Creates a new key-value sink backed by a zip file using the default (identity) mapping between
   * keys and the entry inside the zip used for storing their value. The caller must ensure that the
   * zip file is not closed or modified, otherwise all behavior is undefined. To specify a function
   * that defines the mapping between keys and the zip entry used for their values, see {@link
   * #forZip(File, Function)}.
   *
   ...
   */
  @Nonnull
  public static KeyValueSink<Symbol, byte[]> forZip(
