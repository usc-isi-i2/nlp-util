 ...
 *
 * <p>The input is required to fall into three fields when split by {@link #fieldSplitter()}
 * (defaults to tab-separated). The first of these is expected to be the row key, the second the
 * column key, and the third the value. Multiple values may appear in one value field if the
 * optional {@link #valueListSplitter()} is specified to split them with.
 *
 * <p>Comment lines are currently not skipped, but this could be added as an option in the future.
 *
 * @author Ryan Gabbard
 */
...
public abstract class MultitableLoader<R, C, V> {
  ...
  /**
   * Loads a {@link ListMultitable} from the specified source according to the configuration of this
   * loader.
   */
  public final ImmutableListMultitable<R, C, V> loadToListMultitable(CharSource source)
      throws IOException {
    final ImmutableListMultitable.Builder<R, C, V> ret = ImmutableListMultitable.builder();
  ...
  /**
   * Loads a {@link SetMultitable} from the specified source according to the configuration of this
   * loader.
   */
  public final ImmutableSetMultitable<R, C, V> loadToSetMultitable(CharSource source)
      throws IOException {
    final ImmutableSetMultitable.Builder<R, C, V> ret = ImmutableSetMultitable.builder();
  ...
  private void loadToMultitable(
      final CharSource source, final ImmutableMultitable.Builder<R, C, V> ret) throws IOException {
    source.readLines(
          ...
          public boolean processLine(final String line) throws IOException {
            ...
            if (fields.size() == 3) {
              ...
              if (valueListSplitter().isPresent()) {
                ...
                ret.put(
                    rowKey, columnKey, interpret(fields.get(2), valueInterpreter(), "value", line));
              }
            } else {
  ...
  private <T> T interpret(
      String field, Function<String, T> interpreter, String fieldName, String line)
      throws IOException {
    try {
      return interpreter.apply(field);
    } catch (Exception e) {
      throw new IOException(
          "While parsing multitable line\n"
              + line
              + "\n failed to interpret "
              + fieldName
              + " "
