...
/**
 * Given two parameter files, logs those params only in the left (prefixed by "<"), those only in
 * the right (prefixed by "<"), and those in both with conflicting values (prefixed by "<>").
 *
 * <p>Usage: ParametersDiff [leftParamFile] [rightParamFile]
 */
public final class ParametersDiff {
  ...
  private static void trueMain(String[] argv) throws IOException {
    final SerifStyleParameterFileLoader paramFileLoader =
        new SerifStyleParameterFileLoader.Builder().crashOnUndeclaredOverrides(false).build();
    ...
    dumpOnlyIn(
        "< ",
        Sets.difference(leftParams.asMap().keySet(), rightParams.asMap().keySet()),
        leftParams);
    dumpOnlyIn(
        "> ",
        Sets.difference(rightParams.asMap().keySet(), leftParams.asMap().keySet()),
        rightParams);
    dumpConflicts(leftParams, rightParams);
  }

  private static void dumpOnlyIn(
      final String prefix, final Set<String> uniqueKeys, final Parameters params) {
  ...
  private static void dumpConflicts(final Parameters leftParams, final Parameters rightParams) {
    final List<String> potentialConflicts =
        Ordering.natural()
            .sortedCopy(
                Sets.intersection(leftParams.asMap().keySet(), rightParams.asMap().keySet()));
