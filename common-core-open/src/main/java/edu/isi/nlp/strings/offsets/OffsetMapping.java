...
/**
 * Contains a mapping between of item (typically character) indices from one numbering scheme to
 * another.
 *
 * <p>Define an offset assignment (OA) for a sequence L as a set (U, f) where U is a set of integers
 * such that L_i is not assigned an offset and f is a function from S-U --> integers such that
 * f(L_i) <= f(L_j) for all i<=j where f(L_i) and f(L_j) are defined.
 *
 * <p>This class stores a correspondence between a source offset assignment and a target offset
 * assignment. A canonical example might be between character offsets and EDT offsets in ACE.
 *
 * <p>See @{code AbstractOffsetMapping} a for default implementation.
 */
interface OffsetMapping {
   ...
   *
   * <p>If the range is out-of-bounds in the source, throws an IndexOutOfBoundsException. If the
   * source range is not closed on both ends, throws an IllegalArgumentException. All returned
   * Ranges will be closed on both ends.
   */
