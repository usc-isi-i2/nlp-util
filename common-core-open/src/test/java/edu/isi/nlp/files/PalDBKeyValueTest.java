...
public final class PalDBKeyValueTest {
  ...
  public void testPutAndGet() throws IOException {
    ...
    final ImmutableKeyValueSource<Symbol, ByteSource> source = KeyValueSources.fromPalDB(dbFile);
  ...
  private File createTestDB(final Symbol key, final byte[] value) throws IOException {
    final File dbFile = folder.newFile("test.db");
    try (final KeyValueSink<Symbol, byte[]> sink = KeyValueSinks.forPalDB(dbFile, false)) {
      sink.put(key, value);
    }
    return dbFile;
  }
