...
public class PercentileComputerTest {
  ...
  public void testNIST() {
    ...
    assertEquals(95.1981, nistPercentiles.percentile(0.9).get(), 0.0001);
    assertEquals(12, nistPercentiles.numObservedValues());
    assertEquals(95.1990, nistPercentiles.max().get(), 0.0001);
    assertEquals(95.0610, nistPercentiles.min().get(), 0.0001);
    assertEquals(95.1579, nistPercentiles.median().get(), 0.0001);

    final PercentileComputer.Percentiles wikiPercentiles =
        nistComputer.calculatePercentilesCopyingData(wikipediaData);
    assertEquals(26.0, wikiPercentiles.percentile(0.4).get(), 0.0001);
    assertEquals(35.0, wikiPercentiles.median().get(), 0.0001);
  }
  ...
  public void testExcel() {
    ...
    assertEquals(29, wikiPercentiles.percentile(0.4).get(), 0.0001);
  }
}
