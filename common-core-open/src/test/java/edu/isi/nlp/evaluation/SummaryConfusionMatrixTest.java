...
import static edu.isi.nlp.collections.IterableUtils.zip;
...
public class SummaryConfusionMatrixTest {
  ...
  public void testSummaryConfusionMatrix() {
    final ImmutableList<Symbol> systemAssignments =
        ImmutableList.of(PER, ORG, ORG, PER, PER, LOC, LOC, PER);
    final ImmutableList<Symbol> goldAssignments =
        ImmutableList.of(ORG, LOC, ORG, ORG, PER, LOC, PER, PER);
    ...
    assertEquals(
        3.0D / 8.0D,
        SummaryConfusionMatrices.chooseMostCommonRightHandClassAccuracy(confusionMatrix),
        1e-6);
    ...
    assertEquals(
        4.0D / 8.0D,
        SummaryConfusionMatrices.chooseMostCommonLeftHandClassAccuracy(confusionMatrix),
        1e-6);
    ...
    assertEquals(
        perF, SummaryConfusionMatrices.FMeasureVsAllOthers(confusionMatrix, PER).F1(), 1e-6);
    ...
    assertEquals(
        orgF, SummaryConfusionMatrices.FMeasureVsAllOthers(confusionMatrix, ORG).F1(), 1e-6);
  }
}
