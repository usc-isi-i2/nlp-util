...
/**
 * A bootstrap sampling strategy for any metric computable from a binary confusion matrix. This also
 * supports breaking down the output based on the results of some provided function.
 *
 * <p>An item is consider a true positive if it has an alignment in the key. Each unaligned test
 * item is considered to be a false positive and each unaligned key item is considered to be a false
 * negative. The aggregators are responsible from processing these counts into some score which is
 * bootstrapped.
 *
 * <p>If a breakdown function is specified, separate results will be produced for each output of
 * that function where the alignment is filtered to include only items with the same value from the
 * function (e.g. to break down results by event type).
 */
public final class BinaryConfusionMatrixBootstrapStrategy<T>
    implements BootstrapInspector.BootstrapStrategy<
        Alignment<? extends T, ? extends T>, Map<String, FMeasureCounts>> {

  private final Function<? super T, String> breakdownScheme;
  private final ImmutableSet<BootstrapInspector.SummaryAggregator<Map<String, FMeasureCounts>>>
      summaryAggregators;

  private BinaryConfusionMatrixBootstrapStrategy(
      final Function<? super T, String> breakdownScheme,
      final ImmutableSet<
              ? extends BootstrapInspector.SummaryAggregator<Map<String, FMeasureCounts>>>
          summaryAggregators) {
  ...
  private FMeasureCounts confusionMatrixForAlignment(
    ...
    return SummaryConfusionMatrices.FMeasureVsAllOthers(
        summaryConfusionMatrixB.build(), EvaluationConstants.PRESENT);
  }

  @Override
  public Collection<BootstrapInspector.SummaryAggregator<Map<String, FMeasureCounts>>>
