...
/**
 * If we have some metric broken down by categories which results in F-measure counts, these will
 * print bootstrapped precision/recall/F-measure scores.
 *
 * <p>Each of these is provided with an output directory (<tt>outputDir</tt>) and a name. The
 * following outputs will be written:
 *
 * <ul>
 *   <li>{@code outputDir/name.bootstrapped.txt}: A human readable chart with the botostrapped
 *       percentiles for precision, recall, F1, and accuracy.
 *   <li>{@code outputDir/name.bootstrapped.csv}: the same information in a machine-friendly CSV
 *       form.
 *   <li>{@code outputDir/name.bootstrapped.medians.csv}: CSV for the median bootstrapped values
 *       only.
 *   <li>{@code outputDir/name.bootstrapped.raw}: Machine-consumable list of all the bootstrap
 *       samples for all four metrics. This is useful for passing to gnuplot to make box and whisker
 *       graphs.
 * </ul>
 */
public final class BrokenDownFMeasureAggregator
  ...
  private BrokenDownFMeasureAggregator(String name, File outputDir) {
    ...
    checkArgument(
        !name.endsWith(Character.toString(File.separatorChar)),
        "Aggregation name cannot end with %s, but got %s",
        File.separatorChar,
        name);
  ...
  public void observeSample(final Collection<Map<String, FMeasureCounts>> observationSummaries) {
    ...
    for (final Map.Entry<String, Collection<FMeasureCounts>> aggregate :
        allCountsB.build().asMap().entrySet()) {
      final FMeasureCounts fMeasureInfo =
          FMeasureCounts.combineToMicroFMeasure(aggregate.getValue());
      // we scale scores to 0-100 for easier reading
      f1sB.put(aggregate.getKey(), 100.0 * fMeasureInfo.F1());
      precisionsB.put(aggregate.getKey(), 100.0 * fMeasureInfo.precision());
      recallsB.put(aggregate.getKey(), 100.0 * fMeasureInfo.recall());
    }
  }
