 ...
 */
@Beta
public final class BinaryErrorLogger<KeyT extends HasDocID, TestT extends HasDocID>
  ...
  public void inspect(final Alignment<? extends KeyT, ? extends TestT> alignment) {
    ...
    for (final TestT testItem : alignment.rightAligned()) {
      confusionMatrixBuilder.recordPredictedGold(
          PRESENT,
          PRESENT,
    ...
    for (final TestT testItem : alignment.rightUnaligned()) {
      confusionMatrixBuilder.recordPredictedGold(PRESENT, ABSENT, testRenderer.apply(testItem));
    }
    ...
    try {
      ...
      if (docID.isPresent()) {
        ...
        docDir.mkdirs();
        Files.asCharSink(new File(docDir, "confusionMatrix.txt"), Charsets.UTF_8)
            .write(
                confusionMatrixBuilder
                    .build()
                    .prettyPrintWithFillerOrdering(Ordering.usingToString()));
      }
    } catch (IOException e) {
  ...
  private Optional<Symbol> searchForDocID(
    ...
    Symbol docID = null;
    for (final HasDocID hasDocID :
        Iterables.concat(
            alignment.leftAligned(),
            alignment.leftUnaligned(),
