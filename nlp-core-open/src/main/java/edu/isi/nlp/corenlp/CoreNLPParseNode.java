...
public final class CoreNLPParseNode
  ...
  private CoreNLPParseNode(
      final Symbol tag,
      final List<CoreNLPParseNode> children,
      final Optional<CoreNLPParseNode> head,
      final Optional<CoreNLPToken> token,
  ...
  private static CoreNLPParseNode create(
      final Symbol tag,
      final List<CoreNLPParseNode> children,
      final Optional<CoreNLPParseNode> head,
      final Optional<CoreNLPToken> token,
  ...
  public static CoreNLPParseNode create(
      final Symbol tag,
      final List<CoreNLPParseNode> children,
      final Optional<CoreNLPParseNode> head,
      final Optional<CoreNLPToken> token) {
    ...
    checkArgument(
        !span.isPresent()
            || span.get().startInclusive().asInt() <= span.get().endInclusive().asInt(),
  ...
  public static class CoreNLPParseNodeBuilder {
    ...
    public CoreNLPParseNode build() {
      // we impose no requirement that text or token is present; verbs, noun phrases, etc., can all
      // get dropped and parsers may find where they belonged.
      if (text.isPresent() && token.isPresent()) {
        checkState(
            text.get().equals(token.get().content()),
            "if we have both text and a data they must be the same, but instead we have "
                + text.get()
                + " and "
                + token.get().content());
      }
      ...
      if (children.isEmpty()) {
        ...
        head = headFinder.findHead(tag, ImmutableList.copyOf(children));
      }
