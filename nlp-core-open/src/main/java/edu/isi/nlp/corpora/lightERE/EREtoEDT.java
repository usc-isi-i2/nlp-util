...
public class EREtoEDT {
  ...
  private EREDocument convertDocumentToEDT(
      final EREDocument ereDoc, final ImmutableMap<Integer, Integer> offsetMap) {
  ...
  private Iterable<? extends EREEvent> convertEREEvents(
      final List<EREEvent> events, final ImmutableMap<Integer, Integer> offsetMap) {
    ...
    for (final EREEvent raw : events) {
      ...
      for (final EREEventMention rawM : raw.getEvents()) {
        ...
        if (!rawDate.isPresent()
            || !offsetMap.containsKey(rawDate.get().getDateExtent().getOffset())) {
          ...
          EDTConverted.setDateExtent(
              EREDateExtent.fromPreciseOffsets(
                  rawDateExtent.getSource(),
  ...
  private List<EREArg> convertArgs(final List<EREArg> args) {
    return ImmutableList.copyOf(
        Iterables.filter(
            args,
            new Predicate<EREArg>() {
              @Override
              public boolean apply(final EREArg input) {
                return !deletedOrIgnoredIds.contains(input.getEntityId())
                    && !deletedOrIgnoredIds.contains(input.getEntityMentionId());
              }
            }));
  }

  private Optional<ERETrigger> triggerFromOffset(
      final ERETrigger base, final ImmutableMap<Integer, Integer> offsetMap) {
  ...
  private Iterable<? extends ERERelation> convertERERelations(
      final List<ERERelation> relations, final ImmutableMap<Integer, Integer> offsetMap) {
    ...
    for (final ERERelation raw : relations) {
      ...
      for (final ERERelationMention rawM : raw.getMentions()) {
        ...
        // trigger does not need to be checked for presence here; not required by schema (can be
        // inferred from args)
  ...
  private Iterable<EREEntity> convertEREEntities(
      final List<EREEntity> entities, final ImmutableMap<Integer, Integer> offsetMap) {
  ...
  private Optional<EREEntityMention> convertEntityMention(
      final EREEntityMention rawM, final ImmutableMap<Integer, Integer> offsetMap) {
    final EREEntityMention.NOUNTYPES nountypes;
    if (rawM.getNounType().name().equals("NA")) {
      log.warn(
          "Warning! Mention with NA type! Changing to NOM and ignoring relation! FIXME {}", rawM);
      // ignored
      deletedOrIgnoredIds.add(rawM.getId());
    ...
    return Optional.of(
        EREEntityMention.fromPreciseOffsets(
            rawM.getId(), nountypes, rawM.getSource(), newStart, newEnd, rawM.getText()));
  }

  /**
   * lightERE offsets are indexed into the document, including text inside tags EDT offsets are
   *
   * <p>TODO use the mapping in bue-common-open
   */
