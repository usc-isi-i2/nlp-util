...
public abstract class EDLWriter {
  ...
  /**
   * Writes out the specified {@link EDLMention}s. If a {@link #defaultKbId} was not specified, an
   * exception will be thrown if any EDL mentions have an absent KB ID.
   */
  ...
  private String toLine(final EDLMention edlMention) {
    ...
    checkArgument(
        kbId.isPresent(),
        "EDL mention %s does not have a KB ID and no default was specified",
        kbId);
    return edlMention.runId()
        + "\t"
        + edlMention.mentionId()
        + "\t"
        + edlMention.headString()
        + "\t"
        + edlMention.documentID()
        + ":"
        + edlMention.headOffsets().startInclusive().asInt()
        + "-"
        + edlMention.headOffsets().endInclusive().asInt()
        + "\t"
        + kbId.get()
        + "\t"
        + edlMention.entityType()
        + "\t"
        + edlMention.mentionType()
        + "\t"
