...
/* package-private*/ final class CorefScorerUtils {
  ...
  static void checkPartitionsOverSameElements(
      final Set<Object> predictedItems, final Set<Object> goldItems) {
    if (!predictedItems.equals(goldItems)) {
      final Set<Object> predictedButNotGold = Sets.difference(predictedItems, goldItems);
      final Set<Object> goldButNotPredicted = Sets.difference(goldItems, predictedItems);
  ...
  static List<Set<Object>> toSets(final Iterable<? extends Iterable<?>> iterables) {
    final ImmutableList.Builder<Set<Object>> ret = ImmutableList.builder();
