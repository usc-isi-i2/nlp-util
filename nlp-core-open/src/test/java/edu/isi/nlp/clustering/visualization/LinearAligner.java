package edu.isi.nlp.clustering.visualization;
 ...
 *
 * <p>Note this aligner assumes aligned base items are the same by .hashCode and .equals(). If this
 * is not true, simply define an equivalence relation for your alignment and wrap each item in an
 * {@link com.google.common.base.Equivalence.Wrapper}.
 *
 * <p>This class is experimental and subject to backwards-incompatible changes. Use at your own
 * risk. ~ rgabbard
 */
@Beta
public final class LinearAligner<ItemType> {
  ...
  public Result<ItemType> align(
      Iterable<? extends Iterable<ItemType>> leftClusters,
    ...
    for (int i = 0; i < iterations; ++i) {
      ...
      mutableAlignment.doBestSwap(MutableAlignment.Side.Left, objectiveFunction);
      mutableAlignment.doBestSwap(MutableAlignment.Side.Right, objectiveFunction);
  ...
  public static final class Result<ItemType> {
    ...
    private Result(
        Iterable<ClusterOrdering<ItemType>> left, Iterable<ClusterOrdering<ItemType>> right) {
  ...
  public static class ClusterOrdering<ItemType> implements Iterable<ItemType> {
    ...
    public ClusterOrdering(Iterable<ItemType> originalCollection, List<ItemType> orderedItems) {
      this.originalCollection = originalCollection;
      this.orderedItems = checkNotNull(orderedItems);
    }

    public static <ItemType> ClusterOrdering<ItemType> createFor(Iterable<ItemType> coll) {
  ...
  private static interface ObjectiveFunction<ItemType> {
    ...
    double score(
        Multimap<ItemType, Integer> leftPositions, Multimap<ItemType, Integer> rightPositions);
  }
  ...
  private static class MutableAlignment<ItemType> {
    ...
    public MutableAlignment(
        List<ClusterOrdering<ItemType>> leftOrdering,
    ...
    public static <ItemType> MutableAlignment<ItemType> create(
        ...
        Iterable<? extends Iterable<ItemType>> right,
        boolean insertGaps) {
      ...
      return new MutableAlignment(
          listify(left, leftGapsToInsert), listify(right, rightGapsToInsert));
    }
    ...
    private ImmutableMultimap<ItemType, Integer> positionMultimap(
      ...
      final ImmutableMultimap.Builder<ItemType, Integer> ret = ImmutableMultimap.builder();
    ...
    public void doBestSwap(Side side, ObjectiveFunction<ItemType> objectiveFunction) {
      int orderingSize = side == Side.Left ? leftOrdering.size() : rightOrdering.size();
  ...
  private static class LinearDistance<ItemType> implements ObjectiveFunction<ItemType> {
    ...
    @Override
    public double score(
        Multimap<ItemType, Integer> leftPositionMap, Multimap<ItemType, Integer> rightPositionMap) {
      double score = 0.0;
      ...
      for (final Map.Entry<ItemType, Collection<Integer>> leftPositions :
        ...
        score +=
            bestScore(leftPositions.getValue(), rightPositions)
    ...
    private double bestScore(
        Collection<Integer> primaryPositions, Collection<Integer> secondaryPositions) {
